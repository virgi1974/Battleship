...
describe Board do
  ...
  let(:expected_default_initialized_fields) do
  ...
  describe 'initialization' do
    ...
    it 'creates default content' do
      ...
      %i[a0 b4 c3 d1 e4].each do |expected_key|
        expect(board.fields.keys.include?(expected_key)).to be true
      end
      %i[a5 b100 f0 e5].each do |expected_key|
  ...
  context 'methods' do
    ...
    describe '#create_ship' do
      ...
      it 'updates the board values for the given positions' do
        candidate_positions = %i[a0 a1 a3]
      ...
      it 'returns true' do
        expect(board.create_ship(%i[a0 a1 a3])).to be true
      end

      it 'raises OverWritingError exception in case it tries to write more than once' do
        board.fields[:a0] = 1
        expect(board.fields[:a0]).to eq 1
        expect { board.create_ship(%i[a0 a1 a3]) }.to raise_error(Board::OverWritingError, 'that field has been already updated')
      end

      it 'raises OverWritingError exception in case it tries to write more than once' do
        expect(board.fields.key?(:a10)).to be false
        expect { board.create_ship(%i[a0 a1 a10]) }.to raise_error(Board::OutOfBoardLimitsError, 'field is out of board limits')
      end
    end

    describe '#drop_bomb' do
      it 'returns 0 1 2 integer values on case result' do
        board.ships << %i[a1 a2 a3]
        # missing
